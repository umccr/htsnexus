...
# This utility takes a 'raw' htsnexus index database generated by one of the
# htsnexus_index_{bam,cram,vcf} programs - possibly downsampled - and adds
# a 'seqBin' column to the block index (htsfiles_blocks). This is populated
# with a bin number reflecting each (seqLo,seqHi] range, or null if seq is
...
# More info about binning: https://www.biostars.org/p/100316/
# We use five levels, starting with bins of 16,384bp, increasing by a factor
# of 16 (2^4) at each level. This leads to a max sequence length of 2^30 bp.
#
# We'd really rather have seqBin as a materialized view instead of requiring
# this post hoc transformation, but SQLite does not support that.
